    ...
    $recursively = function (array $utils) use ($regex, &$recursively): void {
        foreach ($utils as $class) {
            if (is_array($class)) {
                ...
                if (!class_exists($class) || !defined($class."::LANGUAGES") || !is_subclass_of($class, "App\Core\Language")) {
                    _html(
                        '<i>env.php</i> > <i>'.$class.'</i><br>' .
                        _code(file_get_contents(str_replace('\\', '/', $class) . '.php')) .
                        _error("ENV translations classes should extend <i>App\Core\Language</i> and contain LANGUAGES array constant.")
                    );
                }
