...
$max_attempts = array(
    'recompile-css-js' => call_user_func(function () {
        $files = File::tree(App\Core\ENV::design('css'));
                                    ...
                                    foreach (['layouts', 'mails', 'outcomes', 'pieces'] as $folder) {
                                        ...
                                        foreach (File::rFolder($folder, ['json']) as $file) {
                                            if (basename($file) == $filename) {
                                                ...
                                                if (!empty($utils['scss']['paths']) && is_array($utils['scss']['paths'])) {
                                                    foreach ($utils['scss']['paths'] as $path) {
                                                        $resources['scss'][$path][] = $file;
                                                    }
                                                }
                                                if (!empty($utils['js']['header']) && is_array($utils['js']['header'])) {
                                                    foreach ($utils['js']['header'] as $path) {
                                                        $resources['js']['header'][$path][] = $file;
                                                    }
                                                }
                                                if (!empty($utils['js']['footer']) && is_array($utils['js']['footer'])) {
                                                    foreach ($utils['js']['footer'] as $path) {
