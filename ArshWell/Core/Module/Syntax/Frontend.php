        ...
        foreach ($actions as $key => $action) {
            foreach ($action as $category => $attributes) {
                foreach ($attributes as $attr => $value) {
                    $actions[$key][$category][$attr] = ("App\Core\Module\Syntax\Frontend\Action\\{$category}")::{$attr}($value);
                }
            }
        }
    ...
    static function features (array $features): array {
        foreach ($features as $key => $feature) {
            foreach ($feature as $category => $attributes) {
                foreach ($attributes as $attr => $value) {
                    $features[$key][$category][$attr] = ("App\Core\Module\Syntax\Frontend\Feature\\{$category}")::{$attr}($value);
                }
            }
        }
    ...
    static function fields (array $fields): array {
        foreach ($fields as $key => $field) {
            foreach ($field as $category => $attributes) {
                foreach ($attributes as $attr => $value) {
                    $fields[$key][$category][$attr] = ("App\Core\Module\Syntax\Frontend\Field\\{$category}")::{$attr}($value);
                }
            }
        }
