...
use App\Core\Tygh\Minifier\JsMin;
...
final class Layout {
    ...
    /* media */
        static function mediaSCSS (string $folder, array $pieces, bool $css = false): array {
            ...
            foreach ($return['json']['layout']['scss']['paths'] as $file) {
            ...
            foreach ($return['json']['outcome']['scss']['paths'] as $file) {
                foreach (glob('resources/scss/'. $file .'.scss') as $f) {
        			$return['files'][] = array(
        				'name'	=> (File::name($f, false) .'.'. $extension)
        			);
            ...
            foreach ($pieces as $piece) {
        		...
        		foreach ($array['json']['scss']['paths'] as $file) {
                    foreach (glob('resources/scss/'. $file .'.scss') as $f) {
        				$return['files'][] = array(
        					'name'	=> (File::name($f, false) .'.'. $extension)
        				);
        ...
        static function mediaJSHeader (string $folder, array $pieces): array {
            ...
            foreach ($return['json']['layout']['js']['header'] as $file) {
            ...
            foreach ($return['json']['outcome']['js']['header'] as $file) {
                foreach (glob('resources/js/'. $file .'.js') as $f) {
                    $return['files'][] = array(
        				'name' => $f
        			);
        	...
        	foreach ($pieces as $piece) {
        		...
        		foreach ($array['json']['js']['header'] as $file) {
                    foreach (glob('resources/js/'. $file .'.js') as $f) {
        				$return['files'][] = array(
        					'name'	=> $f
        				);
        ...
        static function mediaJSFooter (string $folder, array $pieces, bool $nonexistent_in_header = false): array {
        	$return = array(
        		...
        		'files'	=> array(
                    array(
                        'name' => 'DevTools/tools/files/design/js/custom/body.js',
                        'range' => array(
        	                'min' => 0 // guarantees will compile even if there are no other js files
        				)
                    )
        	...
        	foreach ($return['json']['layout']['js']['footer'] as $file) {
        		foreach (glob('resources/js/'. $file .'.js') as $f) {
        	...
        	foreach ($return['json']['outcome']['js']['footer'] as $file) {
        		foreach (glob('resources/js/'. $file .'.js') as $f) {
        			if (!$nonexistent_in_header || !in_array($f, $js_header_files)) {
        				$return['files'][] = array(
        					'name' => $f
        	...
        	foreach ($pieces as $piece) {
        		...
        		foreach ($array['json']['js']['footer'] as $file) {
                    foreach (glob('resources/js/'. $file .'.js') as $f) {
        				if (!$nonexistent_in_header || !in_array($f, $js_header_files)) {
        					$return['files'][] = array(
        						'name'	=> $f
        ...
        static function mediaMailSCSS (string $folder, array $pieces, bool $css = false): array {
            ...
            foreach ($return['json']['mail']['scss']['paths'] as $file) {
    ...
    /* compile */
        ...
        static function compileSCSS (string $folder, array $pieces, string $url = NULL, string $destination = NULL, Table $row = NULL, array $vars = NULL): bool {
            ...
            require_once("App/Core/Tygh/SCSS/autoload.php"); // leafo/SCSS
            ...
            foreach ($mins as $min) {
                ...
                if (!isset($js_web_class_vars)) {
                    ...
                    $js_web_class = JsMin::minify(preg_replace(
                        ...
                        file_get_contents('DevTools/tools/files/design/js/custom/Web.js')
                    ));
                }
        ...
        static function compileMailSCSS (string $folder, array $pieces, string $url = NULL, string $destination = NULL, Table $row = NULL, array $vars = NULL): bool {
            ...
            require_once(Folder::realpath("App/Core/Tygh/SCSS/autoload.php")); // leafo/SCSS
