...
<div class="container margin-3rd-3rd">
    <div class="row">
        <div class="col-sm-7 col-md-8 col-lg-7">
            ...
            if (!empty($_SESSION['appointments'])) { ?>
                <b class="text-color-2 mb-3 d-block">
                    <?= Brain\View\Site::text('programări.făcute', array(
                        'appointments'  => $_SESSION['appointments'],
                        'email'         => '<a href="mailto:'.Brain\Table\Config::title('email').'">'.Brain\Table\Config::title('email').'</a>',
                        'contact'       => '<a target="_blank" href="'.Arsh\Core\Web::url('site.contact').'">CONTACT</a>'
                    )) ?>
                </b>
            <?php } ?>

            <form action="<?= Arsh\Core\Web::url('site.ajax.appoint') ?>">
                <div class="row">
                    <div class="col-md-6 mb-1 mb-sm-2">
                        <label><?= Brain\View\Site::sentence('form.câmp.nume.label') ?> <span class="text-danger">*</span></label>
                    ...
                    <div class="col-md-6 mb-1 mb-sm-2">
                        <label><?= Brain\View\Site::sentence('form.câmp.prenume.label') ?> <span class="text-danger">*</span></label>
                    ...
                    <div class="col-md-6 mb-1 mb-sm-2">
                        <label><?= Brain\View\Site::sentence('form.câmp.email.label') ?></label>
                    ...
                    <div class="col-md-6 mb-1 mb-sm-2">
                        <label><?= Brain\View\Site::sentence('form.câmp.telefon.label') ?> <span class="text-danger">*</span></label>
                    ...
                    <div class="col-md-6 mb-1 mb-sm-2">
                        <label><?= Brain\View\Site::sentence('form.câmp.serviciu.label') ?> <span class="text-danger">*</span></label>
                        <select class="custom-select" name="id_service">
                            <option <?= (empty($form->value('id_service')) ? 'selected' : '') ?> hidden>
                                <?= Brain\View\Site::sentence('form.câmp.serviciu.placeholder') ?>
                            </option>
                            <?php
                    ...
                    <div class="col-md-6 mb-1 mb-sm-2">
                        <label><?= Brain\View\Site::sentence('form.câmp.dată.label') ?></label>
                        <input type="date" class="form-control" name="date" value="<?= $form->value('date') ?>" />
                        <span class="text-danger" form-error="date"></span>
                    </div>
                    <div class="col-12 mb-2">
                        <label><?= Brain\View\Site::sentence('form.câmp.mesaj.label') ?></label>
                        <textarea class="form-control" name="message"
                        placeholder="<?= Brain\View\Site::sentence('form.câmp.mesaj.placeholder') ?>"><?= $form->value('message') ?></textarea>
                ...
                <input type="submit" class="btn-4-7 nowrap" value="<?= Brain\View\Site::sentence('form.submit.text') ?>" />
                <div class="my-2">
            ...
            <hr />
            <?= Brain\View\Site::text('contactează-mă', array(
                'email'     => '<a href="mailto:'.Brain\Table\Config::title('email').'">'.Brain\Table\Config::title('email').'</a>',
                'contact'   => '<a target="_blank" href="'.Arsh\Core\Web::url('site.contact').'">CONTACT</a>'
            )) ?>
        </div>
        <div class="col-sm-5 col-md-4 offset-lg-1">
            <hr class="d-sm-none" />
            <h5 class="text-center text-sm-left mb-3"><?= Brain\View\Site::sentence('servicii') ?></h5>
            <?php
            foreach ($services as $service) { ?>
                <div class="padding-0-2nd">
                    <!-- <a target="_blank" href="<?= Arsh\Core\Web::url('site.coaching.service', ['id'=>$service->id(),'slug'=>$service->translation('title')]) ?>"> -->
                        <?= Arsh\Core\Piece::html('site/service/card', array(
                            'service' => $service
                        )) ?>
                    <!-- </a> -->
                </div>
